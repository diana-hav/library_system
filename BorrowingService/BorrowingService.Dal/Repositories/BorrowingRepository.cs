using BorrowingService.Domain.Entities;
using BorrowingService.Dal.Interfaces;
using Dapper;
using Npgsql;

namespace BorrowingService.Dal.Repositories;

public class BorrowingRepository : IBorrowingRepository
{
    private readonly NpgsqlConnection _connection;
    private readonly NpgsqlTransaction? _transaction;

    public BorrowingRepository(
        NpgsqlConnection connection,
        NpgsqlTransaction? transaction = null)
    {
        _connection = connection;
        _transaction = transaction;
    }

    public Task<IEnumerable<Borrowing>> GetAllAsync()
    {
        var sql = @"SELECT id,
                           reader_id AS ReaderId,
                           book_id AS BookId,
                           borrow_date AS BorrowDate,
                           return_date AS ReturnDate,
                           status AS Status
                    FROM borrowings
                    ORDER BY borrow_date DESC;";

        return _connection.QueryAsync<Borrowing>(sql, transaction: _transaction);
    }

    public Task<Borrowing?> GetByIdAsync(int id)
    {
        var sql = @"SELECT id,
                           reader_id AS ReaderId,
                           book_id AS BookId,
                           borrow_date AS BorrowDate,
                           return_date AS ReturnDate,
                           status AS Status
                    FROM borrowings
                    WHERE id=@id;";

        return _connection.QuerySingleOrDefaultAsync<Borrowing>(sql, new { id }, _transaction);
    }

    public Task<int> AddAsync(Borrowing b)
    {
        var sql = @"INSERT INTO borrowings (reader_id, book_id, borrow_date, status)
                    VALUES (@ReaderId, @BookId, @BorrowDate, @Status)
                    RETURNING id;";
        return _connection.ExecuteScalarAsync<int>(sql, b, _transaction);
    }

    public Task UpdateStatusAsync(int id, string status)
    {
        var sql = @"UPDATE borrowings 
                    SET status=@status,
                        return_date = CASE WHEN @status='returned' THEN NOW() ELSE return_date END
                    WHERE id=@id;";
        return _connection.ExecuteAsync(sql, new { id, status }, _transaction);
    }
}
